...
const TempCTA = ({ head, text, tags }) => {
  ...
  return (
      ...
      <div id='new' className="new_cta font-jost text-[#CBCBCB] bg-gradient-to-b md:bg-gradient-to-r from-[#012023] to-[#254447] rounded-3xl mx-6 py-8 px-2 md:px-0 lg:pl-8 flex flex-col-reverse sm:flex-row-reverse gap-12 -translate-y-12 md:-translate-y-24">
        ...
        <img ref={myRef} src={newcta_phn} alt="doodle" className={`fade-in md:hidden w-5/6 sm:w-2/6 mx-auto drop-shadow-2xl object-cover ${myElementIsVisible ? "appear" : ""}`} />
        ...
        <div className="h-[3rem] md:h-16 xl:h-20 w-4/6 md:w-3/6 rounded-lg md:rounded-2xl bg-gradient-to-r from-[#FAFF00] to-[#7AFF53] p-[2px] ">
          <Link to="contact" spy={true} smooth={true} offset={-100} duration={800}>
            <div className='flex h-full w-full items-center justify-center bg-[#012023] back rounded-lg md:rounded-2xl cursor-pointer hover:-translate-y-1 transition-transform'>
              <span className="cta_price font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#FAFF00] to-[#7AFF53] text-2xl md:text-2xl lg:text-4xl xl:text5xl">₹ 9,999/-</span>
            </div>
          </Link>
            <PopupButton
              ...
              className="py-3 text-sm 2xl:text-lg underline underline-offset-4"
            />
          </div>
        </div>
      </div>
